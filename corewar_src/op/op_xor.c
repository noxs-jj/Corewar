...
/*   Updated: 2015/03/17 19:42:05 by vjacquie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	op_xor_4(t_data *d, t_header *player, unsigned int (*value)[])
{
...
static int	op_xor_3(t_data *d, t_header *player, unsigned int (*value)[])
{
...
int			op_xor(t_data *d, t_header *player)
{
	...
	if (op_xor_4(d, player, &value) < 0)
		return (-1);
	if (op_xor_3(d, player, &value) < 0)
		return (-1);
