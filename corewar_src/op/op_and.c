...
int		op_and(t_data *d, t_header *player)
{
	...
	if (ft_strncmp(player->codage, "01", 2) == 0
		...
		value[0] = reg_to_int(d, player, ft_hex2Dec((char *)player->opArgs[0]));
		// value[0] = ft_hex2Dec(player->reg[ft_hex2Dec((char *)player->opArgs[0])]);
	else
		value[0] = ft_hex2Dec((char *)player->opArgs[0]);
	if (ft_strncmp(&player->codage[2], "01", 2) == 0
		&& isValidRegister(ft_hex2Dec((char *)player->opArgs[1])) >= 0)
		value[1] = reg_to_int(d, player, ft_hex2Dec((char *)player->opArgs[1]));
		// value[1] = ft_hex2Dec(player->reg[ft_hex2Dec((char *)player->opArgs[1])]);
	else
		value[1] = ft_hex2Dec((char *)player->opArgs[1]);
	ft_bzero(player->reg[reg], REG_SIZE);
	ft_bzero(str, REG_SIZE + 1);
