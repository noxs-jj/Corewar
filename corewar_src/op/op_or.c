...
static int	op_or_3(t_data *d, t_header *player, unsigned int (*value)[])
{
...
static int	op_or_2(t_data *d, t_header *player, unsigned int (*value)[])
{
...
int			op_or(t_data *d, t_header *player)
{
	...
	if (op_or_3(d, player, &value) < 0)
		return (-1);
	if (op_or_2(d, player, &value) < 0)
		return (-1);
