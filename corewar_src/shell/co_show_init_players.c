...
void	co_show_init_players(t_data *d)
{
	...
	if (d->players > 0 && (prog = search_prog(d, d->nbrWinner)) != NULL)
		co_info_player1(d, prog);
	if (d->players > 1 && (prog = search_prog(d, d->nbrWinner)) != NULL)
		co_info_player2(d, prog);
	if (d->players > 2 && (prog = search_prog(d, d->nbrWinner)) != NULL)
		co_info_player3(d, prog);
	if (d->players > 3 && (prog = search_prog(d, d->nbrWinner)) != NULL)
